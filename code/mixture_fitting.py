...
def node_init(args):
    ...
    result = op.minimize(nll, [0.0, 1, 0.05], args=(Y[:,1], Y[:,3], Y[:,6]**.5 , xref)
    m_ug, b_ug, lnf_ug = result["x"]
    result = op.minimize(nll, [0.0, 1, 0.05], args=(Y[:,1], Y[:,4], Y[:,10]**.5 , xref)
    m_gr, b_gr, lnf_gr = result["x"]
    result = op.minimize(nll, [0.0, 1, 0.05], args=(Y[:,1], Y[:,5], Y[:,14]**.5 , xref)
...
if __name__ == '__main__':
   ...
   red_file = h5py.File("red_cat_hires.hdf5" , 'r')
   ...
   red_par = h5py.File("red_par_hires.hdf5" , 'w')
