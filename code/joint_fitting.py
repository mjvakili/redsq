...
def lnlike_fg(p):
    ...
    what it really returns is alikelihood vector for now. 
    Not a scalar. 
    y, yerr, var, model: Nx3
    '''
    #m_ref = np.median   
    m1, m2, m3, b1, b2, b3, lnf1, lnf2, lnf3, _, _, _, _, _, _, _  = p
    
    model = np.zeros_like((y))
    model[:,0] = m1 * (x - 19.0) + b1
    model[:,1] = m2 * (x - 19.0) + b2
    model[:,2] = m3 * (x - 19.0) + b3
    
    var = np.zeros_like((yerr))
    var[:,0] = np.exp(lnf1) + yerr[:,0] ** 2
    var[:,1] = np.exp(lnf2) + yerr[:,1] ** 2
    var[:,2] = np.exp(lnf3) + yerr[:,2] ** 2
   
    return -0.5 * ((model - y) **2 / var + np.log(var))

def lnlike_bg(p):
    '''
    likelihood of background (blue) galaxies.
    Note: this returns a Nx3 matrix not a scalar
    it gets summed up in lnprob
    '''
    _, _, _, _, _, _, _, _, _, Q, M1, M2, M3, lnV1, lnV2, lnV3 = p
...
def lnprob(p):
    ...
    '''
    m1, m2, m3, b1, b2, b3, lnf1, lnf2, lnf3, Q, M1, M2, M3, lnV1, lnV2, lnV3 = p
