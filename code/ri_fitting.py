...
def mcmc(args): 
    ...
    fig_file = "/home/vakili/public_html/files/redsequence_ri/"+str(zmin)+"_z_"+str(zmax)+"burn_iter"+str(iteration)+".png"
    ...
    axes[-1].set_xlabel("Step Number", fontsize=25)
    fig.tight_layout(h_pad=0.0)
    fig_file = "/home/vakili/public_html/files/redsequence_ri/"+str(zmin)+"_z_"+str(zmax)+"chain_iter"+str(iteration)+".png"
    plt.savefig(fig_file)
    plt.close()
    ...
    #fig.tight_layout()
    fig_file = "/home/vakili/public_html/files/redsequence_ri/"+str(zmin)+"_z_"+str(zmax)+"color_iter"+str(iteration)+".png"
    plt.savefig(fig_file)
    plt.close()
...
if __name__ == '__main__':
   ...
   for i in range(Nthreads):
       ...
       arglist[i] = (zmin, zmax , 2, x, y , yerr)  
   result = list(mapfn(mcmc_fg, [ars for ars in arglist]))
   
   ##for t in range(Nthreads):
   ##    zmin = z_init + t * 0.02
   ##    zmax = zmin + 0.02
   ##    np.savetxt("results4/ri_result_z_"+str(zmin)+"_"+str(zmax)+".txt" , np.array(result[t]))
   pool.close()
